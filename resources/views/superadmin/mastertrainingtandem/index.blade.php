...
<div class="pagetitle">
  <h1>Karyawan Training & Tandem</h1>
</div><!-- End Page Title -->
<section class="section dashboard">
    <div class="row">
      <div class="col-lg-12">
        <div class="card">
          <div class="card-body">
        <h6 class="mt-2">Import Data Karyawan Training & Tandem</h6>
    <div class="importdata mt-4 mb-4">
    <form id="upload-form" action="{{ route('importkaryawantrainingtandem') }}" method="post" enctype="multipart/form-data" style="display: flex; align-items: center;">
        @csrf  
        ...
        <button class="btn btn-warning" type="submit">Unggah File</button>
            ...
            <table class="table">
              <thead>
                <tr>
                  ...
                  <th scope="col">Kelas Training</th>
            ...
            @foreach ( $karyawantrainingtandem as $item )
            <tr>
                <td>{{$item->nama_karyawan}}</td>
                <td>{{$item->posisi}}</td>
                <td>{{$item->domisili}}</td>
                <td>{{$item->kelas_training}}</td>
                <td>{{$item->tanggal_training}}</td>
                <td>{{$item->status}}</td>
                <td>{{$item->reason}}</td>
                <td>
                <form method="POST" action="{{ route('deletetrainingtandem', $item->id) }}" style="display: inline;">
        @csrf
        @method('DELETE')
