...
<div class="pagetitle">
  <h1>Psikotes (Sedang Dijadwalkan)</h1>
</div>
...
<form method="GET" action="{{ route('superadmin.psikotesesafter.index') }}" class="mb-3 mt-4">
    <div class="row">
    <div class="col-md-4">
...
<div class="button-section">
  <button type="button" class="btn btn-success btn-sm mr-2" 
          onclick="handleAction('lolos', {{ $item->id }})" 
          {{ $item->hasil_status !== 'Dijadwalkan' ? 'disabled' : '' }}>Lolos</button>
  <button type="button" class="btn btn-danger btn-sm mr-2" 
          onclick="handleAction('tidak lolos', {{ $item->id }})" 
          {{ $item->hasil_status !== 'Dijadwalkan' ? 'disabled' : '' }}>Tidak Lolos</button>
  <button type="button" class="btn btn-warning btn-sm mr-2" 
          onclick="handleAction('stop proses', {{ $item->id }})" 
          {{ $item->hasil_status !== 'Dijadwalkan' ? 'disabled' : '' }}>Stop Proses</button>
  <button type="button" class="btn btn-secondary btn-sm mr-2" 
          onclick="handleAction('tidak hadir', {{ $item->id }})" 
          {{ $item->hasil_status !== 'Dijadwalkan' ? 'disabled' : '' }}>Tidak Hadir</button>
</div>
...
<div class="modal fade" id="saveCandidateConfirmModal" tabindex="-1" aria-labelledby="saveCandidateConfirmLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="saveCandidateConfirmLabel">Konfirmasi Pesan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
               <h5> Apakah Anda ingin menyimpan kandidat ini?</h5>
                <p>Jika ya, maka kandidat akan tersimpan dalam halaman Simpan Kandidat. Jika tidak, maka kandidat akan tersimpan dalam halaman Stop Proses</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="proceedWithoutSaving()">Tidak</button>
                <button type="button" class="btn btn-primary" onclick="saveAsCandidate()">Ya</button>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="saveCandidateConfirmModaldua" tabindex="-1" aria-labelledby="saveCandidateConfirmLabeldua" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="saveCandidateConfirmLabeldua">Konfirmasi Pesan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <h5>Apakah Anda ingin menyimpan kandidat ini?</h5>
                <p>Jika ya, maka kandidat akan tersimpan dalam halaman Simpan Kandidat. Jika tidak, maka kandidat akan tersimpan dalam halaman Tidak Lolos</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="proceedWithoutSavingdua()">Tidak</button>
                <button type="button" class="btn btn-primary" onclick="saveAsCandidate()">Ya</button>
            </div>
        </div>
    </div>
...
<script>
  ...
  function handleAction(action, id) {
    ...
    if (action === 'stop proses') {
        // Tampilkan modal bertanya untuk Simpan Kandidat atau Tidak
        new bootstrap.Modal(document.getElementById('saveCandidateConfirmModal')).show();
    } 
    else if (action === 'tidak lolos'){
        new bootstrap.Modal(document.getElementById('saveCandidateConfirmModaldua')).show();
    } else if (action === 'tidak hadir') {
        // Tampilkan modal penjadwalan ulang
...
function saveReschedule() {
    ...
    fetch("{{ route('create-log-tahapanpsikotes') }}", {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
