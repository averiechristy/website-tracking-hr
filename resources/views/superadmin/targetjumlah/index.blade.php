...
<main id="main" class="main">
    <div class="pagetitle">
      <h1>Target MPP & Jumlah Mitra Existing</h1>
    </div><!-- End Page Title -->
    <section class="section dashboard">
        <div class="row">
          <div class="col-lg-12">
            <div class="card">
              <div class="card-body">
                <a href="{{route('superadmin.targetjumlah.create')}}" class="btn btn-sm btn-primary mb-3 mt-3">Tambah Data</a>
                ...
                <table class="table">
                  ...
                  <tbody>
                       ...
                       <tr>
                        <td>{{ \Carbon\Carbon::createFromFormat('m', $item->bulan)->format('F') }}</td>
                        <td>{{$item->tahun}}</td>
                        <td>{{$item->posisi->nama_posisi}}</td>
                        <td>{{$item->target_mpp}}</td>
                        <td>{{$item->jumlah_mitra}}</td>
                        <td>{{$item->target_join}}</td>
                        <td>

                        <a  href="{{ route('superadminshowtargetjumlah', $item->id) }}" class="btn btn-sm btn-warning" data-bs-toggle="tooltip" title="Edit">
                        <i class="bi bi-pencil"></i>
                    </a>

                    <form method="POST" action="{{ route('superadmindeletetargetjumlah', $item->id) }}" style="display: inline;">
    @csrf
    @method('DELETE')
